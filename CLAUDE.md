...
- **Card header styles:** `SCOREBOARD_CARD_HEADER_STYLE` setting (`banner`/`compact`/`wheel`/`sidebar`/`fullart`); compact shows 48x48 thumbnail + title; wheel shows pinball wheel PNG above card border with configurable scale (`SCOREBOARD_WHEEL_SCALE`, 100-200%, default 150%); sidebar shows image left of game title (Stern Insider style); fullart fills entire card with game background image, each element (title, scores, footer, QR) rendered on frosted glass panels
