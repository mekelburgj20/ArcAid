...
- [ ] **Backup restore drill** — DEFERRED (owner, 2026-08-09) to the pre-launch closeout alongside the wipe + version reset; owner will call the timing. Plan: restore a backup off-hours + rehearse the wipe script against the restored copy in the same session.
...
- [ ] **Pre-launch data wipe — SCOPED (owner, 2026-08-09), execution DEFERRED to the very last pre-go-live step.** Scope: **keep ALL rooms** (rooms, games, settings, styles, members), **zero the scores**. Explicitly NOT yet — owner is running another RTX demo first, so live scores stay until then; "that'll be the last thing we do before golive" (i.e., after the demo, immediately before the v1.0.0 bump). Execution plan when triggered: full DB + assets backup first (BackupManager + manual off-box copy), then a deliberate scripted pass over the score tables (`submissions`, `score_history`, `community_scores`, `global_scores`, leaderboard caches, ranking caches, lobby feed score events) — never ad-hoc deletes; verify caches rebuild clean after.
- [x] **Decide: RetroAchievements config** — ✅ CONFIGURED + VERIFIED 2026-08-09. Owner opted ON. Detour: the Global Settings UI had NO fields for `RA_USERNAME`/`RA_API_KEY` (nor the v2.15.0 `OPS_ALERT_*` keys) — FE catalog gap fixed in v2.88.1 (PR #179). Owner saved creds; master-list sync run via admin API: **8,628 games across 20 consoles**; public search verified live (`/api/global/ra-catalogue/search?q=chrono trigger` returns RA results with `inCatalogue:false` → Add-to-Arcaid path armed). Residual: owner should do one real add-from-search when curious (writes a permanent catalogue row, deliberately left manual).

### B. Launch-gating code (~3–4 focused days total)

- [ ] **Public version reset** — root `package.json` → **v0.90.0 Beta**, → v1.0.0 at GA (versioning plan). Surfaces via `/api/version`, Help footer, Dashboard health card. DEFERRED (owner, 2026-08-09) to the pre-launch closeout with the wipe + restore drill — owner calls launch time; keep working Tier 1+ until then. **Pre-launch closeout order: restore drill → score wipe (keep rooms, zero scores) → version reset → go.**
...
- [x] **Kiosk → `ScoreboardSurface` migration** — ✅ SHIPPED v2.90.0 (PR #182, 2026-08-09; parity shots approved). Net −149 lines; programmatic layout diff zero deltas; 3 default-off surface props (zoom, kiosk-only QR, header spacing); 2 documented micro-deltas (legacy+QR-all now shows QR on kiosk; bg scrolls with overflow).
...
- [ ] **Share-to-Arcaid (photo → score submission)** — owner-asked 2026-08-14, wanted before GA, **sequenced AFTER the style-revamp design arc** (owner call, same day). Details in ROADMAP "Share-to-Arcaid". One feature, not three: build the CORE orphan-photo landing flow (room+game chooser — the bulk of the work), then hang three entry points on it — Paste button (all platforms, no install), Android `share_target` (manifest + service-worker POST intercept), iOS Apple Shortcut (**WebKit never shipped `share_target`, so iPhone has no web-native route at all**). ~3–4 focused days all-in.
...
- [x] **Decide `UAT_SCRIPT.md` + `TEST_KNOWN_ROUGH_EDGES.md`** — ✅ DECIDED 2026-08-09: owner chose SKIP (no rehearsal); both archived to `tmp/`. An RTX demo script was written instead (`tmp/rtx-demo-script.md`, 2026-08-09) — the demo itself is a live field test.
- [x] **Discord slash-command ↔ web-feature drift audit** — ✅ AUDITED + ALL 9 FINDINGS FIXED, v2.97.1 (PR #196, 2026-08-10). The owner's drift instinct was right: 6 BROKEN (`/pick-game` dead for standalone/iScored-off rooms — the v2.81 default; `/create-backup`+`/sync-state` ungated; 4 ban-cascade leaks in list-winners/my-stats/view-stats; cross-room write bypass on 4 write commands — new shared `validateDiscordWriteTarget` guard, deny-by-default guild-match; `/setup` was writing pre-multi-room GLOBAL settings w/ a cross-tenant announce-channel leak + a zero-reader admin-role setting → retired to a web-Settings signpost) + 3 DRIFT (deactivate-game re-check, activate-game autocomplete mode filter, reorder-lineup guild-scoping, map-user ban check). 31 regression tests added (1622 total). Prod-verified, commands re-registered.
- [x] **Untracked `.vscode/` + `data/` subdirs** — ✅ DONE 2026-08-08: gitignored `data/catalogue-images|iscored-styles|score-photos|styles/` (runtime assets, ~6GB) + `.vscode/`; deleted empty `data/a.txt`. `data/callouts.json` stayed tracked until 2026-08-23, when it was deleted — the `callouts` table (admin-page upload/edit) is the source of truth.
