...
- [ ] **Discord slash-command ↔ web-feature drift audit** (owner concern, 2026-08-09 pre-RTX-demo). The 21 slash commands haven't been systematically revisited against this sprint's 13 releases. Core commands (submit-score/pick-game/map-user) were touched+tested recently; the read-only listing commands (/list-scores, /list-winners, /view-selection, /view-stats, /mystats) and ops commands (/sync-state, /run-cleanup, /reorder-lineup, /nominate-picker, /pause-pick, /setup) are the least-recently-exercised. Audit: each command vs its web twin (does it respect login-gating, room-scoped ratings, ban cascade, hidden_at comment filters, videogame mode normalization?). Post-demo slot — fold in any drift the demo itself surfaces.
