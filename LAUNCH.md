...
- [ ] **Prod server ops** (post-S3 backlog): ~~add `init: true` to docker-compose~~ ✅ verified already live 2026-08-08 (`/opt/arcaid/docker-compose.yml` has `init: true` + healthcheck, container `Init=true`, 0 zombies on host). REMAINING: run the pending Ubuntu update (8 pkgs: docker-ce 29.6.1 + kernel 6.8.0-134) + reboot — owner must run it (Claude's SSH apt was permission-blocked): `ssh arcaid "export DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a; apt-get update -qq && apt-get -y upgrade && reboot"`
