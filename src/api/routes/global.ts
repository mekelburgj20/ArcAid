 ...
 *
 * Username lock: the request's `displayName` field is IGNORED. This route is
 * `requireDiscordUser`, so the submitter is always authenticated and their name
 * is resolved server-side via `UserProfileService.resolveSubmitName`
 * (user_mappings alias → user_profiles.display_name → JWT username claim → id).
 * Pre-lock, an arbitrary typed name became the scoreboard name AND was
 * registered as a permanent `user_mappings` alias for the account. Renames now
 * go through Account Settings (`PATCH /api/users/me/profile`).
 */
router.post('/global/scores', globalSubmitLimiter, requireDiscordUser, requireNotBanned, globalScoreUpload.single('photo'), async (req, res) => {
    try {
        ...
        const scoreRaw = req.body.score;
        const excludeFromGlobal = req.body.excludeFromGlobal === 'true' || req.body.excludeFromGlobal === true;
        // Username lock: `req.body.displayName` is deliberately NOT read. Older
        // clients still post it; it is ignored rather than rejected.
        ...
        // Username lock — resolve the submitter's canonical name server-side.
        // Precedence (see UserProfileService.resolveSubmitName): existing
        // user_mappings alias > user_profiles.display_name > JWT username claim
        // > raw id. The request body has no say.
        const db = await getDatabase();
