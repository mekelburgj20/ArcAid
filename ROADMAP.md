...
- [ ] **Ban scope** — SHIPPED: global-only (a ban blocks login/room-creation everywhere). NOT shipped: the room-tier ban (user can't submit/comment in ONE room, existing content orphaned) — still open.
...
- [x] **Audit** — every ban / unban / suspend / unsuspend / display-name-override is an authed admin write, so the existing blanket `auditMiddleware` (server.ts) auto-logs actor/action/target for free — no bespoke audit code needed.
