...
3. **Admin action tools (make reports meaningful) — Phase 2, not yet built.** Super-admin: rename room, force display-name reset (ties to the "Admin display-name override" item), **suspend room** (hidden + inaccessible pending review — softer than delete), **ban identity** (block login/creation for a provider id — ties to the "Admin ban" item under Player Self-Service + Moderation, and to `IdentityLinkService.resolveCanonical` enforcement at login/refresh/room-create). A queue without teeth just documents abuse — Phase 1's only "action" is a shortcut link to the existing Game Rooms manager (rename/delete already live there).
