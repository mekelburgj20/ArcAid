...
- [ ] **Enforcement points** — SHIPPED v2.44.0: token issuance (Discord/Google OAuth callback, refresh) + room creation, via `BanService.isIdentityBanned` (checks both directions of an `IdentityLinkService` link). NOT shipped: room submissions, comment POSTs, rating POSTs, friend POSTs — an unexpired access token still works for these until its own short TTL elapses. Extending `isBanned()` to these per-write chokepoints is the next increment.
