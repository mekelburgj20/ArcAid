...
- **Parallel iScored Playwright sessions step on each other on Wed 22:00** — when multiple tournaments' maintenance + scheduled cleanup fire at the same minute, each spins up its own `IScoredClient` against the same iScored account, and Playwright's dropdown state can flip mid-delete. Symptom seen 2026-04-29: WG-VPXS cleanup logged `[WARN] Game 'CSI' not found in dropdown. Skipping delete.` for both CSI and X-Men Wolverine LE — they stayed visible on iScored despite our DB marking them HIDDEN. Two sub-bugs: (a) `runCleanup` (`TournamentEngine.ts:1429`) logs `Deleted from iScored: <name>` after `client.deleteGame()` returns even when the inner method short-circuited with the dropdown warning — caller can't tell success from no-op; should propagate the result; (b) the architectural fix is to share one iScored session per account per cron firing, or serialize all iScored mutations behind a per-account mutex. Investigate together.
