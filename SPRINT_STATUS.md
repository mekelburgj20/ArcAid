...
- **▶ ACTIVE PLAN: execute `tmp/improvement-audit-sprint-plan-v2.md` (25 sprints S0–S24 → public v0.90.0 Beta). Phase 0 (S0–S3) MERGED + deployed (PR #1); node_modules untrack MERGED (PR #2); Phase A / S4 (live scores + viewer rank) MERGED + deployed 2026-06-15 (PR #3); Phase A / S5 (the submit moment) MERGED + deployed 2026-06-15 (PR #22); Phase A / S6 (dethrone loop & notification hygiene) MERGED + deployed 2026-06-16 (PR #23 — ships INERT, flag `NOTIFY_HIGH_VALUE_DEFAULT_ON` OFF); Phase B / S7 (admin data-safety bugs) MERGED + deployed 2026-06-16 (PR #24) — see blocks below. NEXT: S8 (admin guardrail paper-cuts — Phase B).** Standing rule still in force: any sprint touching TournamentEngine/Scheduler/ScoreSyncPoller/TimeoutManager adds regression tests (S2 built the harness). Migration ledger: 102 (S0), 103 (S1), 104 (S3), 105 (S6 — `player_milestones_fired`) consumed; S7 added no migration; **next free = 106**.
- **▸ SESSION SNAPSHOT — end 2026-06-16 (resume from here):** Phase 0 + Phase A are COMPLETE in prod; Phase B underway. **S0–S7 all merged to `main` + deployed** (`main` HEAD = `d7dee5960`). **Resume at S8** (admin guardrail paper-cuts). Open follow-ups — NONE block S8:
  1. **Visual UX pass still pending** (all flagged `review-ux`, unverifiable headless): S4 live toast + kiosk toast; S5 result card + QR row-highlight + guest CTA (user confirmed these 3 work in prod); S7 Rename dry-run preview modal + delete-confirm active/queued list + Paused badge.
