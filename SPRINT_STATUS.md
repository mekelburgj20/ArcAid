...
- **▸ PRIOR SESSION SNAPSHOT — end 2026-06-25 (superseded by the 2026-07-02 resume point below):** Phase 0 + Phase A COMPLETE in prod; Phase B underway. **S0–S9 merged to `main` + deployed.** This session: **shipped S9** — push → PR **#31** → merge (`1282371b1`) → Hetzner deploy run **28148467147** green in 1m55s (vitest gate passed, image pushed to ghcr.io, SSH deploy ran); local `main` fast-forwarded; feature branch deleted. (Earlier in this arc: post-S8 toggle-dirty fix PR #26; in-app-version ROADMAP note PR #27.)
  ...
  - **SESSION 2026-07-05 — Dependabot SAFE WAVE shipped (merged + deployed to prod, green):**
    ...
    - **⏳ PENDING user verification (the one thing not confirmable from here):** trigger a real prod Playwright op — admin **Reconcile iScored** (Game States page) or `/sync-state` — to confirm `chromium.launch()` works on 1.61.1. Image boots (healthcheck green) but a live browser launch wasn't exercised. High confidence (matched version tags, verified base image, container healthy) but not yet proven.
  - **NEXT — the remaining PARKED items, then S10:**
    1. **Dependabot waves** — **SAFE GROUP SHIPPED 2026-07-05** (#28 checkout, #30 admin-ui-minor-patch, #40 backend-minor-patch — the backend group PR is **#40**, not "#29" as earlier written here). **REMAINING → low/med majors, then sqlite3 solo.** Majors (each its own PR): typescript 6 (#9 backend, #11 admin-ui), vite 8 (#15), eslint 10 (#12), lucide-react 1 (#16), react-easy-crop 6 (#13), uuid 14 (#8 backend, #19 admin-ui), globals 17 (#14), @types/node 25 (#17), @vitejs/plugin-react 6 (#18), @types/bcryptjs 3 (#6). Merging the safe group re-triggered the PR gate on all of them vs new `main` — some go red (typescript 6 / eslint 10 / vite 8 typically need config+code adaptation), some compile clean; triage each on its own branch. Then **`#7 sqlite3 5→6` SOLO, last** (native module, whole DB layer, untestable on Windows — back up prod DB + watch the deploy). Can't regen lockfiles on Windows → drive via `@dependabot recreate/rebase` + the gate. **NB: if a future backend-minor-patch group re-includes `playwright`, bump the Dockerfile base-image tag in the same PR (Playwright coupling gotcha).**
