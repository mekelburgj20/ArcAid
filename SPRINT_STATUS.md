...
- **▸ PRIOR SESSION SNAPSHOT — end 2026-06-25 (superseded by the 2026-07-02 resume point below):** Phase 0 + Phase A COMPLETE in prod; Phase B underway. **S0–S9 merged to `main` + deployed.** This session: **shipped S9** — push → PR **#31** → merge (`1282371b1`) → Hetzner deploy run **28148467147** green in 1m55s (vitest gate passed, image pushed to ghcr.io, SSH deploy ran); local `main` fast-forwarded; feature branch deleted. (Earlier in this arc: post-S8 toggle-dirty fix PR #26; in-app-version ROADMAP note PR #27.)
  ...
  - **SESSION 2026-07-05 — Dependabot SAFE WAVE shipped (merged + deployed to prod, green):**
    ...
    - **MAJORS — still remaining:** BACKEND majors (#6 @types/bcryptjs 3, #8 uuid 14, #9 typescript 6 — all gate-green) as the next validated branch, then **#7 sqlite3 5→6 SOLO/LAST** (native module, whole DB layer — back up prod DB + watch the deploy).
  - **NEXT — the remaining PARKED items, then S10:**
    1. **Dependabot waves** — **SAFE GROUP SHIPPED 2026-07-05** (#28 checkout, #30 admin-ui-minor-patch, #40 backend-minor-patch — the backend group PR is **#40**, not "#29" as earlier written here). **ADMIN-UI MAJORS SHIPPED 2026-07-05 (PR #43** — see the session entry above; #11–#19 closed superseded). **REMAINING: backend majors, then sqlite3 solo.** Backend majors (all gate-green): #6 @types/bcryptjs 3, #8 uuid 14, #9 typescript 6 — consolidate into one validated branch. Then **`#7 sqlite3 5→6` SOLO, last** (native module, whole DB layer, untestable on Windows — back up prod DB + watch the deploy). Can't regen lockfiles on Windows → drive via `@dependabot recreate/rebase` + the gate. **NB: if a future backend-minor-patch group re-includes `playwright`, bump the Dockerfile base-image tag in the same PR (Playwright coupling gotcha).**
