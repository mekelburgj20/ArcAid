...
- **▶ ACTIVE PLAN: execute `tmp/improvement-audit-sprint-plan-v2.md` (25 sprints S0–S24 → public v0.90.0 Beta). Phase 0 (S0–S3) MERGED + deployed (PR #1); node_modules untrack MERGED (PR #2); Phase A / S4 (live scores + viewer rank) MERGED + deployed 2026-06-15 (PR #3); Phase A / S5 (the submit moment) MERGED + deployed 2026-06-15 (PR #22); Phase A / S6 (dethrone loop & notification hygiene) MERGED + deployed 2026-06-16 (PR #23 — ships INERT, flag `NOTIFY_HIGH_VALUE_DEFAULT_ON` OFF); Phase B / S7 (admin data-safety bugs) MERGED + deployed 2026-06-16 (PR #24); Phase B / S8 (admin guardrail paper-cuts) MERGED + deployed 2026-06-16 (PR #25); Phase B / S9 (backup / restore / ops floor) COMMITTED on branch `phase-b-s9-backup-restore` @ `cea139b3f` — NOT yet pushed/merged/deployed (awaiting user go-ahead) — see blocks below. NEXT: S10 (room-admin observability + alerting — Phase B), AFTER S9 ships.** Standing rule still in force: any sprint touching TournamentEngine/Scheduler/ScoreSyncPoller/TimeoutManager adds regression tests (S2 built the harness). Migration ledger: 102 (S0), 103 (S1), 104 (S3), 105 (S6 — `player_milestones_fired`) consumed; S7/S8/S9 added no migration (S9 uses `BACKUP_*` global settings keys, not a schema change); **next free = 106**.
- **▸ SESSION SNAPSHOT — end 2026-06-24 (resume from here):** Phase 0 + Phase A COMPLETE in prod; Phase B underway. **S0–S8 merged to `main` + deployed.** This session also shipped a post-S8 fix (toggle dirty-tracking round-trip, PR #26, merged+deployed) and a docs-only ROADMAP entry for in-app version display (PR #27, merged `[skip ci]`).
  - **⏸ RESUME EXACTLY HERE — S9 is built, committed, and Docker-verified, but NOT shipped.** Branch **`phase-b-s9-backup-restore`** — S9 feature commit **`cea139b3f`** + this status commit on top, **committed locally, NOT pushed** (branch exists only on this machine; a fresh session resumes by checking it out). **The real backup→restore test PASSED** (`docker compose build` clean = full backend tsc; then an isolated in-container `createBackup→verifyBackup→restoreBackup→pruneBackups` run, ALL PASSED — VACUUM-INTO snapshot WAL-safe, integrity_check ok, seeded row survived restore into a throwaway DB, prune floor holds). The test script lives at `tmp/s9-backup-restore-test.cjs` (gitignored; re-run via: `docker compose build` then `docker create --name s9test --entrypoint node arcaid-arcaid:latest /tmp/test.cjs` + `docker cp tmp/s9-backup-restore-test.cjs s9test:/tmp/test.cjs` + `docker start -a s9test` — set `MSYS_NO_PATHCONV=1` in Git Bash or the `/tmp` arg gets mangled).
  - **▶ NEXT ACTION when user says go:** `git push -u origin phase-b-s9-backup-restore` → open PR → merge to `main` → the Hetzner deploy auto-fires (that Docker build is the prod compile gate; it just passed locally) → watch the deploy green → sync `main` + delete branch. THEN S9 is done and the next sprint is **S10**.
  - Open follow-ups — NONE block shipping S9:
  1. **Visual UX pass still pending** (all flagged `review-ux`, unverifiable headless): S4 live toast + kiosk toast; S5 result card + QR row-highlight + guest CTA (user confirmed these 3 work in prod); S7 Rename dry-run preview modal + delete-confirm active/queued list + Paused badge; S8 suppressed-scores list + Remove, the unsaved-changes indicator / Save-gating, read-only Game Room fields, and the dirty-state nav guard (confirm it doesn't over-trigger); S9 (once shipped) the super-admin Backups page — schedule/retention form, Verify/Download/Restore (type-to-confirm) row actions. The S9 docker-compose healthcheck is also unverified live locally (the full app won't boot against the un-migratable dev DB; it WILL be exercised on the prod deploy where the DB migrates clean).
  ...
  4. **Post-S3 prod ops backlog** (memory `project-post-s3-ops-backlog`): delete `arcaid.db.bak-preS3-2026-06-15` once FK enforcement has been stable a few days; add `init: true` to `docker-compose.yml` (Playwright/Chromium zombie reaper); Ubuntu `apt upgrade` + reboot.
...
- **Phase B / S9 — Backup / restore / ops floor — BUILT + COMMITTED + DOCKER-VERIFIED on branch `phase-b-s9-backup-restore` @ `cea139b3f`; NOT yet pushed/merged/deployed (awaiting user go-ahead, end 2026-06-24):**
