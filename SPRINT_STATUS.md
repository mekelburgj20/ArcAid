...
- **▶ ACTIVE PLAN: execute `tmp/improvement-audit-sprint-plan-v2.md` (25 sprints S0–S24 → public v0.90.0 Beta). Phase 0 (S0–S3) MERGED + deployed (PR #1); node_modules untrack MERGED (PR #2); Phase A / S4 (live scores + viewer rank) MERGED + deployed 2026-06-15 (PR #3); Phase A / S5 (the submit moment) COMPLETE on PR #5 (off main), NOT yet merged — see blocks below. NEXT: S6 (dethrone loop & notification hygiene).** Standing rule still in force: any sprint touching TournamentEngine/Scheduler/ScoreSyncPoller/TimeoutManager adds regression tests (S2 built the harness). Migration ledger: 102 (S0), 103 (S1), 104 (S3) consumed; S5 added no migration (read-only helper); **next free = 105**.
