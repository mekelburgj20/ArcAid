...
- **Most recent arc (v2.9.0 — per-row score moderation + multi-slot picker correctness):**
  ...
  - **Cron race fix.** Daily Grind on rtx_pinball had `0 22 * * *` maintenance + `0 22 * * 3` cleanup. On Wednesdays both fired simultaneously; cleanup's first SELECT ran before maintenance had completed today's active game, leaving one game per Wed stuck `COMPLETED` (Black Knight 2000 on 2026-04-29 was the latest casualty). Fix: `runMaintenanceWork` runs cleanup inline at the end of the slot loop when `cleanupRule.mode === 'scheduled'` AND `cleanupCronMatchesNow(cleanupRule.cron, cleanupRule.timezone)`. New private cron-field matcher supports `*`, single ints, comma lists, inclusive ranges. Separate cleanup cron is left registered — fires idempotently after.
  - **Global websocket emit.** `emitLeaderboardUpdated` was defined but never called and was scoped `io.to('game:gameId')` — no FE page joins per-game rooms, so it would have reached zero clients. Both delete endpoints now broadcast globally so other open Scoreboard / admin Leaderboard / Game Detail tabs repaint without a manual reload.
  - **Per-slot picker correctness.** When one user won multiple slots in a single maintenance run (e.g. WG-VPXS max=2 with the same player on top of both), pre-fix we emitted exactly one `[Pending Pick]` placeholder + one DM, collapsing wins. Symptom 2026-04-29: user won X-Men Wolverine LE + CSI; only one slot got refilled, the other sat empty. Dedup rescoped from `(tournament_id, picker_discord_id)` to `(tournament_id, picker_discord_id, won_game_id)`. Embed + DM now name the won game per slot. `/pick-status` extended with `pick_slot_id`/`won_game_id`/`won_game_name`; `Picks.tsx` keys by `pick_slot_id` (was `tournament_id`, collided on multi-slot pendings) and renders `won {game}` next to the tournament name.
  - **Discord `/pick-game` placeholder fulfillment.** Pre-fix the command branched only on `hasOpenSlot` — never checked for an outstanding `[Pending Pick]`. With an open slot the placeholder dangled as stale `QUEUED`; with all slots full the new game appended to the queue tail while the placeholder sat at the front. Three branches now mirror the web route: pending+full → `UPDATE` the placeholder's name (preserves NULL `queue_order` so it stays front-of-queue); open-slot → drop placeholder if present + `activateGame` in one txn; no-pending+full → `queueGame` (unchanged). Embed adds a third "queuedFromPick" path so the user can tell whether their pick jumped the queue (won-pick reward) vs landed at the tail.
  - **Tests.** 129/129 passing. No new test files — handlers exercise paths covered by existing route smoke tests; the cron-field matcher is best verified by the inline-cleanup behaviour next Wed at 22:00 Central.
