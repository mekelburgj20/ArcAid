...
- **▸ PRIOR SESSION SNAPSHOT — end 2026-06-25 (superseded by the 2026-07-02 resume point below):** Phase 0 + Phase A COMPLETE in prod; Phase B underway. **S0–S9 merged to `main` + deployed.** This session: **shipped S9** — push → PR **#31** → merge (`1282371b1`) → Hetzner deploy run **28148467147** green in 1m55s (vitest gate passed, image pushed to ghcr.io, SSH deploy ran); local `main` fast-forwarded; feature branch deleted. (Earlier in this arc: post-S8 toggle-dirty fix PR #26; in-app-version ROADMAP note PR #27.)
  ...
  - **NEXT — the remaining PARKED items, then S10:**
    1. **Dependabot waves** — 16 open PRs (#6–#30). #36's gate now validates them once rebased onto main. Plan: **safe group** (`#28` checkout 6→7 [needs `@dependabot recreate` — deploy.yml now has TWO checkout steps], `#29` backend-minor-patch group, `#30` admin-ui-minor-patch group) → **low/med majors** (typescript 6, vite 8, eslint 10, lucide 1, react-easy-crop 6, uuid 14, @types) → **`#7 sqlite3 5→6` SOLO, last** (native module, whole DB layer, untestable on Windows — back up prod DB + watch the deploy). Can't regen lockfiles on Windows → drive via `@dependabot recreate/rebase` + the gate.
