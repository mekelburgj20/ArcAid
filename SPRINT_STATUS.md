...
- **Current version:** **v2.5.2** — deployed to production 2026-04-26.
- **Most recent arc (v2.5.0 → v2.5.2):**
  - **v2.5.0** — **VR + Steam-pinball taxonomy + score-platform stratification + per-room contribution flow.** One coordinated bundle (score-platform required end-to-end, partial deploys would reject every submit). 7 new platform IDs (`pinball_fx_classic` replaces `pinball_fx3` for Zen rebrand; `pinball_fx_classic_vr`, `pinball_fx_midnight`, `pinball_fx_vr`, `star_wars_pinball_vr`, `zaccaria`, `zaccaria_vr`). Removed legacy `pinball_fx3` + generic `vr` bucket. New `'VR'` `PLATFORM_GROUPS` quick-pick. New `SteamPinballImportService` with curated `PACK_CONTENTS` map (78 packs → 220 tables, source-of-truth `tmp/pack-contents-draft.md`, regenerable via `tmp/emit-pack-data-ts.js`); skip-list catches Volume/Pack/Bundle/Tables/VR/Soundtrack/Editor/Mode entries; `cleanTableName` strips ™/®/©/℠ + wrapping quotes; `findSuffixVariantMatch` folds "X" / "X Pinball" duplicates pre-upsert. Required `platform` field on `submissions`/`score_history`/`community_scores`/`global_scores`; `resolveSubmittablePlatforms(gamePlatforms, tournamentRules?)` helper; `ensurePlatformAllowed` server-side re-validation at every submit handler. `SubmissionSheet` picker (1 platform → chip, 2+ → required dropdown). Discord `/submit-score` auto-fills when 1, rejects with valid-choices reply when 2+. `ScoreSyncPoller` stamps `tournament.iscored_default_platform` on synced rows. Leaderboard endpoint accepts `?platform=<id>`; `RankedEntry` carries per-row platform; "All" view shows badges + demotes NULL-platform rows to a "Platform unknown" tail. Removed legacy "Import VPS" / "Import VPXS Wizard" buttons from per-room library page. New `GlobalGameService.findCandidates` (read-only dedup walker extracted from `upsert`) powers `/proposals` / `/use_global` / `/room_only` / `/submit_to_global`. CSV import switched to two-step preview/commit. Super-admin Catalogue Approvals queue at `/admin/catalogue/approvals` with nav badge polling `/pending-count` every 60s. Public `GET /global/games` hard-codes `status='approved'` (was honoring `?status=` — leak risk). Migrations 083–087. Production import: 152 imported / 198 updated / 78 packs expanded / 0 errors. Bug surfaced + fixed inline: 4 sites used `gl.id = grgl.game_library_id` but `game_library`'s PK is `name` and FK is `game_name` — caught only by tests, would have failed at runtime.
  - **v2.5.1** — **Library = global catalogue (step 1) + display polish.** Per-room library page reads `global_games WHERE status='approved'` directly; legacy `game_room_game_library` curation overlay no longer consulted for the list view. Tournaments still use `game_library` (out of step-1 scope). Step 2 documented in `docs/step-2-cleanup-plan.md`. Submission picker, GameDetail tabs, GameDetail per-row badges, GlobalGameDetail leaderboard now render `displayName` via new `admin-ui/src/lib/platforms.ts` helper (mirrors backend `CANONICAL_PLATFORMS` + `PLATFORM_ALIASES`). Display names shortened: `"Pinball FX Classic"` → `"FX Classic"`, etc. (IDs unchanged, display-only). New "Platform" column on Global Game Detail leaderboard; `GlobalLeaderboardService.recalculate` SELECTs `gs.platform` and propagates through `GlobalRankedEntry`. Migration 088 flushes both leaderboard caches.
  - **v2.5.2** — **Platform-data normalization sweep.** Migration 089 walks every JSON platform array (`global_games.platforms`, `game_library.platforms`, `game_room_game_library.custom_platforms`, `tournaments.platform_rules`) and folds each entry through `normalizePlatform()`. Production landed: 5550 rows normalized. `GameLibrary.tsx` chips + filter row also alias-fold + dedupe at render time as defense in depth.
- **Build state:** backend `tsc` clean, admin-ui `vite build` clean, **119/119 tests pass**. Last verified end of v2.5.2 deploy.
