...
- **Current version:** **v2.10.0** — code complete, **NOT YET DEPLOYED**. Last released to prod: v2.9.0 on 2026-04-29.
...
- **Most recent arc (v2.10.0 — IScored session registry, eliminate parallel-Playwright contention):**
  ...
  - **One-off prod cleanup pending.** Paranormal (iScored ID 95735) and Attack from Mars (95586) are currently iScored orphans (visible there, no local DB row) from the pre-fix bug. Need manual delete via iScored UI as `mekelburgj@gmail.com`. The fix prevents future occurrences but doesn't reconcile existing orphans.
- **Build state:** backend `tsc` clean, **129/129 tests pass**. Admin UI not built (no FE changes). SW `CACHE_NAME` unchanged at `arcaid-v44`.
