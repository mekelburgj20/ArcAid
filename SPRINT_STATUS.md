...
- **▸ PRIOR SESSION SNAPSHOT — end 2026-06-25 (superseded by the 2026-07-02 resume point below):** Phase 0 + Phase A COMPLETE in prod; Phase B underway. **S0–S9 merged to `main` + deployed.** This session: **shipped S9** — push → PR **#31** → merge (`1282371b1`) → Hetzner deploy run **28148467147** green in 1m55s (vitest gate passed, image pushed to ghcr.io, SSH deploy ran); local `main` fast-forwarded; feature branch deleted. (Earlier in this arc: post-S8 toggle-dirty fix PR #26; in-app-version ROADMAP note PR #27.)
  ...
  - **SESSION 2026-07-05 — Dependabot SAFE WAVE shipped (merged + deployed to prod, green):**
    ...
    - **#7 sqlite3 5→6 — pre-validated + DEFERRED (NOT merged, 2026-07-05).** Per the user's "validate in containers first" call, tested sqlite3 6.0.1 install + a real DB open (via the `sqlite` wrapper) on both Docker-stage environments. **Result: alpine/musl ✅ (prebuild loads, DB works) but glibc ❌** — sqlite3 6.0.1's linux-x64 glibc prebuild **requires `GLIBC_2.38` + `GLIBCXX_3.4.31`**, and the prod production base (`playwright:v1.61.1-jammy`) is **glibc 2.35** → the prebuild installs but **fails to `dlopen` at runtime** → the app would crash on DB init at boot (healthcheck fail → restart loop). **No CI gate catches this** (the gate runs on ubuntu-latest, new enough). Enable paths: (a) bump the prod base `v1.61.1-jammy`→`v1.61.1-noble` (Ubuntu 24.04, glibc 2.39 — tag verified; a whole-OS bump needing its own validation) in the SAME PR; (b) source-compile sqlite3 in the prod stage (add build tools + `npm rebuild sqlite3 --build-from-source`; image bloat). **Recommended: DEFER** — sqlite3 5 works fine, this is a hygiene bump. #7 left open, not merged. See ROADMAP.
  - **NEXT — the remaining PARKED items, then S10:**
    1. **Dependabot waves** — **SAFE GROUP SHIPPED 2026-07-05** (#28 checkout, #30 admin-ui-minor-patch, #40 backend-minor-patch — the backend group PR is **#40**, not "#29" as earlier written here). **ADMIN-UI MAJORS SHIPPED 2026-07-05 (PR #43** — see the session entry above; #11–#19 closed superseded). **BACKEND MAJORS SHIPPED 2026-07-05 (PR #44** — see session entry above; #6/#8/#9 closed superseded). **#7 sqlite3 5→6 — DEFERRED/BLOCKED** (pre-validated 2026-07-05, NOT merged): sqlite3 6.0.1's glibc prebuild needs `GLIBC_2.38` but the jammy prod base is glibc 2.35 → runtime `dlopen` failure → prod crash on boot. Enable via a `v1.61.1-noble` base bump (its own validation) or source-compile — see the session entry above + ROADMAP. **Dependabot backlog otherwise CLEARED — 15 of 16 PRs shipped this session** (safe wave + admin-ui majors + backend majors). **NB: if a future backend-minor-patch group re-includes `playwright`, bump the Dockerfile base-image tag in the same PR (Playwright coupling gotcha).**
