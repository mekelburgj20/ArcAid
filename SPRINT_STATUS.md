...
- **▸ PRIOR SESSION SNAPSHOT — end 2026-06-25 (superseded by the 2026-07-02 resume point below):** Phase 0 + Phase A COMPLETE in prod; Phase B underway. **S0–S9 merged to `main` + deployed.** This session: **shipped S9** — push → PR **#31** → merge (`1282371b1`) → Hetzner deploy run **28148467147** green in 1m55s (vitest gate passed, image pushed to ghcr.io, SSH deploy ran); local `main` fast-forwarded; feature branch deleted. (Earlier in this arc: post-S8 toggle-dirty fix PR #26; in-app-version ROADMAP note PR #27.)
  ...
  - **⏸ RESUME POINT — updated 2026-07-02 (this is the authoritative "where we are"):** The user redirected to clear the **pre-S10 B–D follow-up piles** (SPRINT_STATUS open follow-ups + the untracked-doc backlog) BEFORE S10. That arc is **DONE — 4 PRs shipped, all merged + deployed to prod 2026-07-02, final deploy run green:**
  ...
  - **NEXT — the remaining PARKED items, then S10:**
    ...
    2. **Catalogue dup review** — `CATALOGUE_DUP_REVIEW.md` (untracked, repo root), 60 groups, all `Decision: _____`. §2 (37, community/digital IPDB-sharing) = mechanical `strip-ipdb id=<recreation-row>`. §1 (23, year disagreements) = pick IPDB-authoritative year then `merge target=<row>` (needs IPDB lookups). Draft-for-approval (non-destructive); execution mutates the catalogue → user sign-off first. See memory `project-pre-s10-backlog-docs`.
    3. **S10** — room-admin observability + alerting (the originally-next sprint, deferred behind these follow-ups). Plan grounded earlier: poller `getStatus()` (add `lastSuccessAt`/`lastError`), `GET /:roomId/admin/health`, real Dashboard health card, maintenance-run trail, alerting on N poller failures, in-app version.
  - **User prod tasks (their side):** flip `NOTIFY_HIGH_VALUE_DEFAULT_ON` in the new Global Settings toggle (still OFF); delete the 3 old 3.1GB backups after #38's mirror builds; confirm server compose init+healthcheck applied (`docker inspect arcaid --format '{{json .State.Health}}'`); Ubuntu `apt upgrade`.
