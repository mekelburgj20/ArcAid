...
- **Mystery Award** [Player] — Canvas-based random game picker with an animated DMD dot-matrix display and a pinball-cabinet aesthetic (chrome bezel, glowing Fire/Queue action buttons, LED "Tournament Pool" topper above the backbox). Discord-authenticated users can add the selected game straight to their pick queue. The pick/award flow is controlled per tournament by its "Winner picks next game" setting (v2.56.0 — the room-level `ENABLE_GAME_PICK_AWARD` toggle was removed); a room where no tournament has it on hides the Picks nav entry.
...
- **Per-room Settings** [Room Admin] — Room name/slug, Discord guild/role/channel, iScored credentials, timezone, platforms, cooldown/pick-window durations, theme, branding, kiosk options, card style, and Arcaid Chat Responses (see below).
- **Arcaid Chat Responses** [Room Admin / Player] — The bot answers ordinary Discord chat: table callouts plus ten live answers (what's active, time left in the round, current leaders, the asker's own rank, whose pick it is and how long they have, the tournament's platform rules and cooldown, links to Picks and the scoreboard, how to submit a score, how to claim an iScored name). The trigger/response list is global (super admin); each room opts in, chooses which of the four categories it wants (Helpful answers / Game callouts / Banter / Easter eggs), which channels the bot may speak in, and a per-channel cooldown for the fun categories. Players can silence it for themselves from `/arcaid-notifications`, Account Settings, or by saying "Arcaid, shush" in chat.
