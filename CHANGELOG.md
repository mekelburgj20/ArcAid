...
- **Hero restack** — motto moved to sit UNDER the delta (94% of the logo box, below the triangle tip) and the ticker tiles now slide under the MOTTO instead of under the triangle (pull-up reduced to `clamp(-18px, -2.5vw, -8px)`). Fixes the squished mobile stack where motto/wordmark/tiles crowded each other.
...
- **Mobile nav brand fixes** — the room name previously had `min-w-0` + truncate with no floor, so flexbox crushed it to nothing (or 2 chars) on phones, jamming the logo against the first nav item. Now: JS-capped at 12 chars, `min-w-[76px]` floor (~7 chars always readable, full name in `title`), logo `h-6` on phones, bar-level `overflow-x-auto` as the safety valve on the narrowest screens.
