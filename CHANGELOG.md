...
- **New shared pieces:** `components/ThemePicker.tsx` (one `<select>`, one option list — the four hand-rolled copies had already drifted on labels and null options) and `lib/routeClass.ts` (`isAdminPath` / `isGlobalPath` / `getRoomSlugForPath`, lifted out of `ThemeProvider` so "am I on a room page?" is decided once).
