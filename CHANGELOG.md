...
### Added
- **`scoreboard_prefs.roomThemes` — the per-room theme override, keyed by room.** A `roomId -> ThemeId` map stored as a third top-level key beside `desktop`/`mobile` in the existing `user_preferences.scoreboard_prefs` JSON (no migration, no new table), with `GET /api/me/room-themes` and `PUT /api/me/room-themes/:roomId`. It replaces `scoreboard_prefs[device].UI_THEME`, which was the only "theme for this room" that ever existed and was **not per room at all**: one choice applied to every room on one device, and to no room on the viewer's other devices. Per-device prefs and room themes now round-trip through each other untouched in both directions — including Reset All, which posts `null` for every per-device key. A pre-v2.132 `UI_THEME` override is **lifted onto the room the viewer is looking at**, once, by `PreferencesService.getRoomThemes(userId, roomId)` (idempotent, and it sweeps the key from both device blobs so a second device can't re-lift it somewhere else).

### Changed
- **The theme model is one resolution order, written down in a comment block at the top of `ThemeProvider.tsx`.** On a room's public pages: this-room override (`scoreboard_prefs.roomThemes[roomId]`) → **personal theme** (`/me/preferences.ui_theme`; NULL = "use each room's default") → room default (`game_room_settings.UI_THEME`) → `dark`. On admin pages: personal theme → `dark`. Global pages are polarity-only, unchanged. Appearance (v2.130.0) is still applied LAST on every branch. The move is that the personal theme — previously the "admin theme", and admin-pages-only — now outranks the room default on room pages too, which is what makes a single picker enough. Guests are untouched: no personal theme, so room default → appearance. New no-flash mirrors `arcaid-theme-personal` (removed when the theme is NULL) and `arcaid-theme-room-<slug>` — deliberately a different key from the room-default mirror `arcaid-theme-public-<slug>`, which the admin Settings page writes, so changing a room's default can't turn into a personal override for that admin. `/me/preferences` and `/me/room-themes` hydration are both authoritative in either direction, so a value cleared on another device clears here.
- **"Scoreboard display" in the user menu is now "Display settings", and shows on every page.** The sheet is mounted app-wide by the new `DisplaySettingsHost` (it used to belong to the Scoreboard page, which is why the item could only appear there); the page now just listens for a `display-settings-saved` event to re-merge overrides. Retitled and re-grouped into three labelled sections: **Appearance** (Dark/Light/Auto), **My theme** (the personal picker, with "Use each room's default"), and **This room** (the per-room "Theme for this room only" picker, then the existing per-device scoreboard preferences — card style, layout, zoom, Advanced). Every scoreboard preference key, the Save/Reset-All behaviour and the per-device split are unchanged; off-room the third section, the device toggle and the Save button are hidden, because Save posts `null` for every scoreboard key and would clear overrides it never loaded. All three theme controls save the instant they change (the room one is no longer part of the Save button's per-device payload — `UI_THEME` is now posted as an unconditional `null` there, sweeping out the retired key); Reset All clears the room theme too.
- **Room Settings → Theme is a single "Room default theme" field**, captioned "What visitors see unless they pick their own theme in Display settings." Same `UI_THEME` key, same save path. The page no longer writes `/me/preferences` at all, and no longer seeds the personal theme from the legacy room-scoped `ADMIN_THEME` setting (a room setting silently rewriting a person's own account preference).
- **Super-admin Global Settings' theme picker is relabelled "My theme"** and is now the same shared `ThemePicker` component writing the same personal preference. The deprecated "Global Pages Theme" field is untouched.
- **New shared pieces:** `components/ThemePicker.tsx` (one `<select>`, one option list — the four hand-rolled copies had already drifted on labels and null options), `lib/routeClass.ts` (`isAdminPath` / `isGlobalPath` / `getRoomSlugForPath`, lifted out of `ThemeProvider` so "am I on a room page?" is decided once) and `lib/roomThemes.ts` (the player-token client for the room-theme endpoints).

### Removed
- **The "Admin Theme" field on the room Settings page.** It never edited the room — it wrote the signed-in admin's own `/me/preferences.ui_theme` from a page whose every other control is room-scoped. That preference now lives in Display settings and Account settings, and governs this admin's view of the room's public pages as well as their admin pages.
...
### Added
- **Pre-mutation snapshots** at all seven destructive iScored paths (`runMaintenanceInternal`, standalone `runCleanup`, `deleteGameCompletely`, `unpinGameFromScoreboard`, both admin game deletes, the reconcile POST). Each reuses the caller's already-open session (no extra login) and is **debounced per iScored account to one per 10 minutes** (`ISCORED_SNAPSHOT_DEBOUNCE_MS`), so the Wed 22:00 five-tournament wave on rtx_pinball writes exactly one. A failed capture logs at ERROR and the mutation proceeds — tournaments running beats snapshots existing. Game list comes from the authenticated `getGamesOnIScored` (one in-page fetch); scores from the public `getAllScores` (one HTTP call). Photos, per-game style and per-score ids are deliberately not captured.
- **Nightly sweep** at 04:00 (`BOT_TIMEZONE`) over every iScored-enabled account, forced past the debounce, followed by retention pruning (`ISCORED_SNAPSHOT_RETENTION_DAYS`, default 30; the newest file per gameroom is never deleted). Kill-switch `ISCORED_SNAPSHOTS_ENABLED` (global setting or env) — defaults **on**.
- **Super-admin "iScored snapshots" section on the Backups page**: list (gameroom · captured · reason · games · scores · a *partial* flag when either read failed), **Snapshot now**, Download, Delete, and **Restore**. Restore is two-step: a dry run reads the live game list and shows what is missing (pre-ticked) vs already present; Execute recreates each ticked game inside a serialized session — create → tags (one call per tag) → open → replay each player's best score → apply the snapshot's hidden/locked state → **re-link the local `games.iscored_id`** so the poller picks the game back up. The modal states the four structural losses before the button: new iScored ids, score dates become restore time, photos are not restored, only per-player bests come back.
- `GET/POST /api/admin/iscored-snapshots`, `GET …/:gameroom/:name/download`, `DELETE …/:gameroom/:name`, `POST …/:gameroom/:name/restore` (super-admin; delete/restore audited). `getIScoredAccounts()` in `iscoredCreds.ts` — one entry per distinct iScored account with its room ids.

### Changed
- `ScoreSyncPoller.normalizeScoreResponse` extracted verbatim to `src/utils/iscoredScores.ts` (`normalizeIScoredScoreResponse`) so the poller and the snapshot share one normalisation rule; behaviour and log lines unchanged.

### Tests
- New `iscored-snapshots.test.ts` (31): capture shape + orphan scores + partial flags (including the empty-list-twice-while-scores-exist case) · debounce (same/different account, force, partial does not arm, zero window) · prune · list · restore planning (apostrophe/case-insensitive matching, per-player bests, `gameIds` filter, local-row re-link) · execute (tag-per-call, open-before-replay, rejected scores counted, one failure does not strand the rest, Playwright fallback) · path-traversal guard at service and route · super-admin gate · maintenance hook fires with reason `maintenance` before slot work. Under vitest the capture is inert unless a test opts in via `setRootForTests` — the first full-suite run otherwise hit the real iScored API through unrelated maintenance tests using the developer's `.env` creds.
...
- **`studio` — who published the table on the Legends platform.** Sourced from AtGames' own per-publisher eStore collections. `atgames.us` is a stock Shopify storefront, so this needs no scraping: seven collection handles, each served as JSON, with the pack → table mapping inside each product's "Tables included" list.
  ...
  - Attribution runs strongest-evidence-first, five tiers deep: a pack that names the table (208) → a series rule for packs that don't publish contents but stamp the series on the name, e.g. "(Dr. Seuss)" (12) → a pack-title prefix, since AtGames names mini-pack tables after their pack (12) → the brand the name itself announces, e.g. "Williams™ Pinball: FunHouse™" (8) → a curated override map for the residue (39). **279 of 279 tables — 100%.**
  - The four derived tiers generalise — a future "Star Wars Pinball Legends Mini Pack" whose tables are named `Star Wars Pinball: <x>` is attributed with no code change. The curated map (`atgamesStudioOverrides.ts`) does not: it's a snapshot of packs that publish prose instead of a contents list. So the gap is made **loud** — every sync logs the names of tables nothing attributed, which is the maintenance trigger for that file. The override tier runs LAST, so if AtGames ever adds a contents list to one of those packs the store wins automatically and the entry goes inert.
  - **AtGames provisions a table into the catalogue feed before it goes on sale**, so an upcoming release exists as a row with no store listing of any kind for a few weeks. That is the recurring shape of the override file's maintenance, and it self-corrects: the listing appears on release, a derived tier resolves it, and the interim entry goes inert. Three such tables are covered from AtGames' published Originals schedule (Firefighter: Urban, City Golf, Wild Games).
...
- **AtGames dedup ignored storefront decoration, creating 82 duplicate rows.** `upsert` matched on AtGames' name verbatim, and `normalizeGameName` knows nothing about a `Williams™ Pinball:` brand prefix or an `(Natural History)` series label — so those names matched nothing and inserted fresh rows beside the ones already in the catalogue. A new `dedup_name` input runs the step-4 walk against the undecorated name; the row is still STORED under AtGames' own name, so this can only change which existing row an import RECOGNISES, never a row's identity.
  - **`(Pinball)` is deliberately never stripped.** AtGames ships a pinball TABLE and an emulated arcade ROM of the same licence, and they are different games on different engines — `Space Invaders (Pinball)` is not Bally's 1980 machine. Test-pinned, because it reads like decoration and isn't.
- **An AtGames original could be stamped onto a machine that merely shares its name.** AtGames' own `Teenage Mutant Ninja Turtles` landed on Data East's 1991 machine, giving a physical pin an `atgames_id`, a studio and `atgames_native`. A new `original_work` input makes step 4 refuse any candidate that claims a maker.
  - The importer derives it from whether the pack carries a licensed brand: Zen's Williams ports, FarSight's Gottlieb and Magic Pixel's Zaccaria are recreations and may fold into the machine row; everything else is original and may not.
  - **Blank, not "virtual-only", is the test.** The first version of the guard reused `isVirtualOnlyManufacturer`, which counts `Original` as virtual-only — so TMNT skipped both real machines and landed on a VPX community remix instead. `Original` is a real answer to "who made this"; only a blank manufacturer is not.
- **A misspelt manufacturer forked a row on every sync.** The VPXS Wizard README spells Zaccaria as "Zacarria" on one table, and that cannot be fixed at the source — so `manufacturerYearAgree` refused the match and re-created the duplicate on each wizard run. A narrow `MFG_SPELLING_ALIASES` map now canonicalises known misspellings for identity comparison ONLY; nothing rewrites a stored value, and the correct spelling maps to itself so an upstream fix changes nothing. Once the rows meet, upsert's supplied-wins write repairs the spelling on its own.
  - Deliberately misspellings and punctuation variants only — never two companies that merely relate to each other. A wrong entry silently merges two different machines.

### Database
- **Migration 148** adds `global_games.atgames_id` (partial UNIQUE index) and `global_games.studio`.

Tests: backend 1802 (+26), admin-ui 846.
