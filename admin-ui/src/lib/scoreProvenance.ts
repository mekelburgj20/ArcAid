/**
 * Engine + Device score provenance — FRONTEND MIRROR of
 * `src/utils/scoreProvenance.ts` (ADR 0016).
 *
 * Everything below this header is identical to the backend file and is guarded
 * by `src/__tests__/scoreProvenance-parity.test.ts`, which imports both modules
 * and deep-compares the engine ids, device ids, display labels, the
 * engine→device compatibility map and the legacy platform map. That test FAILS
 * on any drift — unlike the older `platforms.ts` mirror, which silently rotted
 * to 22 of 53 aliases with nothing guarding it.
 *
 * Edit the backend file first, then copy the body across. Keep this file
 * dependency-free so the two copies can stay identical.
 */
